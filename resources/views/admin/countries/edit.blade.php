            ...
            <div class="row mb-2">
                ...
                <div class="col-sm-6">
                    <ol class="breadcrumb float-sm-right">
                        ...
                        <li class="breadcrumb-item active">{{__('public.Edit_Country')}}</li>
                    </ol>
                </div>
            </div>
        </div>
                    ...
                    <div class="card">
                        <div class="card-body bg-ytr">
                            ...
                            <div class="tab-pane ladre" id="settings">
                                <form method="post" action="{{route('country-update')}}" enctype="multipart/form-data" class="form-horizontal">
                                    @csrf
                                    <input type="hidden" name="_method" value="Patch" />
                                    <input type="hidden" name="id" class="form-control {{ $errors->has('body')? 'is-invalid':''}} " value="{{old('id',$country->id)}}">

                                    <div class="form-group row">
                                        <label for="" class="col-sm-12 col-form-label">{{__('public.Name')}}</label>
                                        <div class="col-sm-12">
                                            <input type="text" name="name" class="form-control" placeholder="{{__('public.Name')}}" value="{{old('name',$country->name)}}">
                                        </div>
