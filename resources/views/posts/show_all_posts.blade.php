...
<?php //$user = \App\Models\Blacklist::where(['to_id' => $post->user_id, 'from_id' => Auth::user()->id])->first(); ?>
    @if(empty(get_block($post->user_id)))
    
    @if(!empty($post->relUser))
      ...
      <div class="col-sm-12 col-12 post-comment">
        <form action="#" method="POST" class="add_comment_form" id="myFrm{{ $post->id }}" enctype="multipart/form-data">
          ...
          <div class="gter text-center emoji-picker-container">
            ...
            <input class="form-control" type="text" name="comment" data-emojiable="true" id="comments" value="{{old('comment')}}" autocomplete="off">
...
<script>
  ...
  $(document).on('submit', ".add_comment_form", function(e) {
    ...
    var thiss = $(this);
    thiss.closest('.post-comment').find("#add_comment_btn").text("{{ __('public.Adding') }}");
    ...
    $.ajax({
      ...
      success: function(response) {
        ...
        if (response.status == 200) {
          ...
          thiss.closest('.post-comment').find("#add_comment_btn").text("{{ __('public.Publish') }}");
