		...
		<div class="col-lg-7 col-md-7 col-sm-7">
			...
			<div class="coment bg-standard-grey mt-5">
				<div class="row heading">
					...
					<div class="col-md-6 col-sm-6 col-12 btn_secrets_new text-right">
						<div class="bta">
						    @if($secret->status!='pending')
                    		 ...
                    		 @else
    							...
    							onclick="addAccept({{$secret['id']}})" id="add_accept_btn{{$secret->id}}">{{__('public.Accept')}}</a>
    							@else
                                <button type="submit" class="mt-3 border-0 rounded bg-standard-violet text-white  px-2 py-2">
                                {{__('public.Accepted')}}
                                </button>
    							...
    							<a href="javascript:void(0)" type="button" class=" mt-3 rounded bg-standard-violet text-white  px-2 py-2"
    							onclick="addReject({{$secret['id']}})" id="add_reject_btn{{$secret->id}}">{{__('public.Reject')}}</a>
    							@else
                                <button type="submit" class="mt-3 border-0 rounded bg-standard-violet text-white  px-2 py-2">
                                {{__('public.Rejected')}}
                                </button>
...
<script>
            ...
            $.ajax({
                ...
                success: function(response) {
					if (response.status == 200) {
                        $("#add_accept_btn"+id).text("{{ __('public.Accepted') }}");
   ...
   function addReject(id) {
            ...
            $.ajax({
                ...
                success: function(response) {
					if (response.status == 200) {
                        $("#add_reject_btn"+id).text("{{ __('public.Rejected') }}");
