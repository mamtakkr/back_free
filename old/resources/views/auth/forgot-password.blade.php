      ...
      <div class="container">
        <div class="row">
          <nav>
            ...
            <div class="menu">
              <ul>
                <li><a class="bg-standard-violet rounded-left text-white px-2 py-2 active" href="{{route('login')}}">{{__('Login')}}</a></li>
                <li><a class="bg-standard-violet rounded-right  text-white px-2 py-2" href="{{route('registerType')}}">{{__('Register')}}</a></li>
              </ul>
            </div>
          </nav>
        </div>
        ...
        <div class="login-form false">
          <div class="user-credentials ">
            ...
            <form method="POST" action="{{ route('password.email') }}" class="louy login-form ">
                @csrf
                  <p style="color:#fff;">{{ __('Forgot your password? No problem. Just let us know your email address and we will email you a password reset link that will allow you to choose a new one.') }}</p>
                <!-- Email Address -->
                ...
                <div class="form-group text-center mt-3">
                 <input type="submit" value="{{ __('Email Password Reset Link') }}" class="button mb-3 loci button d-block pt-1 pb-1 w-100 h-auto bg-standard-violet base-radius text-white text-capitalize font-weight-normal mx-auto">
              </div>
            </form>
